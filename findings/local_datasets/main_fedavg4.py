...
from sim.data.data_utils import BaseDataset
...
parser.add_argument('--way', default=1, type=int, help='way')
args = parser.parse_args()

# nohup python tests/main_fedavg.py -m mlp -d mnist -s 1 -R 100 -K 10 -M 500 -P 10 --partition exdir --alpha 2 10 --optim sgd --lr 0.05 --lr-decay 0.9 --momentum 0 --batch-size 20 --seed 1234 --log Print &
...
def build_dataset(dtype='mnist', data_path = '../datasets/'):
    #if dtype == 'mnist':
    ...
    # elif dtype == 'cinic10':
    #     train_dataset, test_dataset = dataset_cinic10(data_path)
...
class FedDataset(object):
    def __init__(self, overallset, net_dataidx_map) -> None:
        ...
        for i in range(len(net_dataidx_map)):
            # Modify for faster speed [2023-10-07]
            # https://github.com/SMILELab-FL/FedLab/blob/master/fedlab/contrib/dataset/partitioned_cifar10.py
            # CWT, K=20, M= 10, wvgg9k4, cifar10: 547s -> 102s
            # CWT, K=20, M=500, wvgg9k4, cifar10: 69s  -> 43s
            if way == 1:
...
def customize_record_name(args):
    ...
    record_name = f'FedAvg-IV{way}_M{args.M}_P{args.P}_K{args.K}_R{args.R}_{args.m}_{args.d}_{partition}'\

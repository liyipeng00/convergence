    ...
    def __init__(self, overallset, net_dataidx_map) -> None:
        ...
        for i in range(len(net_dataidx_map)):
            ...
            if way == 1:
                ...
                subset = [overallset[j] for j in net_dataidx_map[i]]
                self.fedsets.append(Subset(subset, range(len(net_dataidx_map[i]))))
...
def customize_record_name(args):
    ...
    record_name = f'CWT-IV{way}_M{args.M}_P{args.P}_K{args.K}_R{args.R}_{args.m}_{args.d}_{partition}'\
...
def main():
    ...
    train_dataset, test_dataset = build_dataset(dtype=args.d)
